...
class TestBulkDeleteFunctionality:
    ...
    async def test_bulk_delete_performance(
        ...
        assert execution_time < 2.0, f"Bulk delete took {execution_time:.2f} seconds, expected < 2.0"
    ...
    async def test_bulk_delete_maintains_data_integrity(
        ...
        delete_result = await entry_service.bulk_delete_profile_configurations(config_ids, test_superuser)
