...
class ManufacturingTypeResolver:
        ...
        *,
        ...
        if create_if_missing:
            ...
            raise NotImplementedError(
                f"Manufacturing type '{name}' not found. Run setup script to create it."
            )
    ...
    async def get_default_for_page_type(
        ...
        if page_type not in cls.VALID_PAGE_TYPES:
            raise ValueError(
                f"Invalid page_type '{page_type}'. Must be one of: {cls.VALID_PAGE_TYPES}"
            )
        ...
        stmt = select(ManufacturingType).where(ManufacturingType.is_active == True).limit(1)
        result = await db.execute(stmt)
    ...
    async def verify_profile_setup(
        ...
        stmt = select(AttributeNode).where(
            AttributeNode.manufacturing_type_id == mfg_type.id, AttributeNode.page_type == "profile"
        )
        result = await db.execute(stmt)
        ...
        critical_indicators = {
            "has_basic_info": any(
                field in actual_fields for field in ["name", "title", "product_name"]
            ),
            "has_type_info": any(
                field in actual_fields for field in ["type", "category", "product_type"]
            ),
            "has_material_info": any(field in actual_fields for field in ["material", "materials"]),
            "has_dimensions": any(
                field in actual_fields for field in ["width", "height", "length", "size"]
            ),
            "has_pricing": any(
                field in actual_fields
                for field in ["price", "cost", "price_per_meter", "price_per_unit"]
            ),
        }
        ...
        rich_indicators = {
            ...
            "sufficient_fields": len(attribute_nodes)
            >= 10,  # At least 10 fields for meaningful configuration
        }
